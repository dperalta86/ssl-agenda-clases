...
function renderNextCard(c) {
  ...
  const tipoClass = c.tipo?.includes("virtual") ? "badge-virtual" : "badge-presencial";
  const tipoLabel = c.tipo?.includes("virtual") ? "Virtual" : "Presencial";
...
function renderPastCard(c) {
  ...
  const miniLinks = linksConUrl.map(r =>
    ...
    <div class="past-card">
      <div class="past-date-col">
        ...
        <div style="margin-top:5px">
          <span class="badge ${tipoClass}" style="font-size:10px;padding:3px 7px">
            ${c.tipo?.includes("virtual") ? "Virtual" : "Presencial"}
          </span>
        </div>
      </div>
